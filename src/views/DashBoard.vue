<template>
  <div class="min-h-screen flex flex-col mt-4">
    <!-- Header -->
    <header class="bg-gradient-to-r from-blue-800 to-indigo-900 text-white shadow-lg rounded-box">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-green-500">
          <div class="flex items-center">
            ...
            <div>
              <h3 class="text-gray-500 text-sm font-medium">Total Revenue (5 days)</h3>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500">
          <div class="flex items-center">
            ...
            <div>
              <h3 class="text-gray-500 text-sm font-medium">Total Orders (5 days)</h3>
                    ...
                    <td class="py-3 px-4">
                      <span
                        :class="{
                          ...
                          'bg-green-100 text-green-800': txn.status === 'Confirmed',
              ...
              <div v-for="(user, index) in topUsersBySpending" :key="user.userId"
                ...
                <div class="flex-grow">
                  ...
                  <div class="w-full bg-gray-200 rounded-full h-2.5 mt-2">
                    <div class="bg-indigo-500 h-2.5 rounded-full" :style="{width: (user.totalSpent / topUsersBySpending[0].totalSpent * 100) + '%'}"></div>
                  </div>
                </div>
...
export default {
  ...
  try {
    const response = await axios.get('http://127.0.0.1:8000/api/v1/admin/inventory/low-stock?limit=5');
    ...
    async fetchSalesSummary() {
      try {
        const response = await axios.get('http://127.0.0.1:8000/api/v1/admin/sales/last-five-days');
...
async fetchTopUsers() {
  try {
    const response = await fetch('http://127.0.0.1:8000/api/v1/admin/customers/top?limit=5');
...
,
  ...
  try {
    const response = await fetch('http://127.0.0.1:8000/api/v1/admin/orders/recent?limit=5');
    const data = await response.json();
...
async fetchCategorySummary() {
  try {
    const response = await fetch('http://127.0.0.1:8000/api/v1/admin/inventory/summary');
    const data = await response.json();
